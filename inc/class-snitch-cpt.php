...
class Snitch_CPT {
	...
	public static function views_edit( $views ) {
		$links = array(
			...
			'support' => '<a href="https://classicpress.net/support/plugin/snitch" target="_blank" rel="noopener noreferrer">' . esc_html__( 'Support', 'snitch' ) . '</a>',
		);
	...
	public static function delete_items( $offset = 0 ) {
		...
		$wpdb->query(
			sprintf(
				...
				// phpcs:ignore ClassicPress.DB.PreparedSQL.NotPrepared -- allow sub query.
				$subquery
			)
		);
